...
public class ACPCompiler {
     ...
     *
     * @param modulesPath the directory that is a root of directories containig ACP modules.
     */
    ...
    public void compile(CompilationSettings settings) {
        ...
        for (String libraryImport : context.project.getLibraryImports()) {
            ...
            if ((libraryModule == null) || (!(libraryModule instanceof Library))) {
                throw new CompilationException(
                        "Library " + libraryImport + " imported by program is not a library module.");
            }
        }
        ...
        if (!(outIncludes.exists() && outIncludes.isDirectory())) {
            throw new CompilationException(
                    "Output directory for includes (" + outIncludes.getAbsolutePath() + ") cannot be created.");
        }
        ...
        if (!(outSources.exists() && outSources.isDirectory())) {
            throw new CompilationException(
                    "Output directory for source files (" + outIncludes.getAbsolutePath() + ") cannot be created.");
        }
        ...
        List<ACPContentGenerator> contentGenerators = new ArrayList<>();
        ...
        // Generate header file for supporting EEPROM items
        ACPEepromDataGenerator acpEEPROMDataGenerator = new ACPEepromDataGenerator();
        acpEEPROMDataGenerator.addDependecy(acpProjectCodeGenerator);
        contentGenerators.add(acpEEPROMDataGenerator);
     ...
     *
     * @param namesOfModules the names of modules forming an initial set of modules required by
     *                       the project.
     */
    private Map<String, Module> loadModulesWithDependencies(Set<String> namesOfModules) {
        Map<String, Module> result = new HashMap<>();
        Set<String> missingModules = new HashSet<>(namesOfModules);
            ...
            for (String requiredModule : missingModules) {
                ...
                try {
                    ...
                    if (!requiredModule.equals(module.getName())) {
                        throw new CompilationException(
                                "Invalid name of module in module description: " + requiredModule);
                    }
                } catch (Exception e) {
     ...
     * @throws CompilationException if the module is not available.
     */
     ...
     *
     * @param components the list of investigated components.
     */
     ...
     * @param componentType the description of component type.
     * @param platform      the target hardware and compilation platform.
     */
    private void checkComponentConfiguration(Component component, ComponentType componentType, Platform platform) {
        ...
        for (Map.Entry<String, PropertyType> entry : componentType.getProperties().entrySet()) {
            ...
            if (!(platform.checkValue(propertyType.getType(), effectiveValue)
                    && propertyType.checkRestrictions(effectiveValue))) {
        ...
        for (String propertyName : component.getProperties().keySet()) {
            if (!componentType.getProperties().containsKey(propertyName)) {
                throw new CompilationException(
                        "Property '" + propertyName + "' is not supported in component '" + component.getName() + "'.");
            }
        }
        ...
        for (String eventName : component.getEvents().keySet()) {
            if (!componentType.getEvents().containsKey(eventName)) {
                throw new CompilationException(
                        "Event '" + eventName + "' is not supported in component '" + component.getName() + "'.");
            }
        }
    }
    ...
    private void copyDirectory(File source, File dest) {
        ...
        if (!(dest.exists() && dest.isDirectory())) {
            throw new CompilationException(
                    "Directory " + dest.getAbsolutePath() + " does not exists or cannot be created.");
        }
    ...
    /**
     * Copies all files from source directory to destination directory in such a way
     * that all files will be merged in the destination directory.
     *
     * @param source     the source directory.
     * @param dest       the destination directory.
     * @param filePrefix the prefix added to all files from the source directory.
     */
    ...
    private void copyFile(File source, File dest) {
        ...
        try {
            ...
            throw new CompilationException(
                    "File " + source.getAbsolutePath() + " cannot be copied to " + dest.getAbsolutePath() + ".");
        }
    }
